...
indoorDataRouter.post(
  ...
  async (request, response) => {
    ...
    try {
      ...
      const indoorData = await IndoorData.findOne({
        name: buildingName.replace(/\s+/g, ""),
      });
      if (!indoorData) {
        console.log("not found for buildign");
        return response
          .status(404)
          .json({ error: "Indoor data not found for building" });
      }
      ...
      if (featureIndex === -1) {
        ...
        return response
          .status(404)
          .json({ error: "Room not found in indoor data" });
      }
      ...
      response
        .status(200)
        .json({ message: "Update request approved and indoor data updated" });
    } catch (error) {
...
indoorDataRouter.post(
  ...
  async (request, response) => {
    try {
      ...
      response
        .status(200)
        .json({ message: "Floor plan request approved and floor plan added" });
    } catch (error) {
      console.error("Error approving floor plan request:", error);
      response
        .status(500)
        .json({ error: "Failed to approve floor plan request" });
    }
  },
);
...
indoorDataRouter.post(
  ...
  async (request, response) => {
    try {
      ...
      response
        .status(200)
        .json({ message: "Floor plan request declined and removed" });
    } catch (error) {
      console.error("Error declining floor plan request:", error);
      response
        .status(500)
        .json({ error: "Failed to decline floor plan request" });
    }
  },
);
...
indoorDataRouter.get("/get-update-requests", async (request, response) => {
  try {
    ...
    users.forEach((local_user) => {
      if (
        local_user.floorPlanRequests &&
        local_user.floorPlanRequests.length > 0
      ) {
...
indoorDataRouter.patch("/:name/:id", async (request, response) => {
  ...
  try {
    ...
    if (id < 0) {
      return response.status(404).send("id must be a non-negative integer");
    }
    ...
    const feature = indoorData.features.find(
      (item) => item.properties.id === id,
    );

    if (!feature) {
      return response.status(404).send("feature with given id not found");
    }

    feature.properties.DestinationCount += 1;
    // console.log(
    //   "Updated destination count of room",
    //   feature.properties.RoomName,
    //   "to",
    //   feature.properties.DestinationCount,
    // );
...
indoorDataRouter.get("/:name/:floor/topRooms", async (request, response) => {
  ...
  try {
    ...
    const topRooms = [];
    const maxRooms = Math.min(3, sortedRooms.length);
...
indoorDataRouter.post("/update-request", async (request, response) => {
  ...
  try {
    ...
    response
      .status(200)
      .json({ message: "Update request submitted successfully" });
  } catch (error) {
    console.error("Error handling update request:", error);
    response.status(500).json({ error: "Failed to submit update request" });
  }
...
indoorDataRouter.delete("/update-request/:id", async (request, response) => {
  ...
  try {
    ...
    response
      .status(200)
      .json({ message: "Update request declined and deleted successfully" });
  } catch (error) {
