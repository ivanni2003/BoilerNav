...
const FeaturePopup = ({ feature, isFeaturePopupOpen, setFeatures, features, linkSelectedFeatures, setIsFeaturePopupOpen, imageRenderProps }) => {
  ...
  const styleTop = feature.geometry.y * imageRenderProps.scale + imageRenderProps.y;
  const styleLeft = feature.geometry.x * imageRenderProps.scale + imageRenderProps.x;

  // If the popup is too close to an edge, put it in the middle of the screen.
  // Else put it at the feature's location.

  const svgWidth = 2 * imageRenderProps.x + imageRenderProps.width;
  const svgHeight = 2 * imageRenderProps.y + imageRenderProps.height;
  const leftMarginEdge = svgWidth * 0.25;
  const rightMarginEdge = svgWidth * 0.75;
  const topMarginEdge = svgHeight * 0.25;
  const bottomMarginEdge = svgHeight * 0.75;
  ...
  // Check if style position is too close to an edge
  if (styleTop < topMarginEdge) {
    style.top = `${topMarginEdge}px`;
    style.transform = 'translate(-50%, -50%)';
  }
  if (styleTop > bottomMarginEdge) {
    style.top = `${bottomMarginEdge}px`;
    style.transform = 'translate(-50%, 0%)';
  }
  if (styleLeft < leftMarginEdge) {
    style.left = `${leftMarginEdge}px`;
  }
  if (styleLeft > rightMarginEdge) {
    style.left = `${rightMarginEdge}px`;
  }
...
const RenderRooms = ({ rooms, onRoomClick, selectedFloorPlan, isEditModeOn, imageRenderProps, route, markedRoom }) => {
  ...
  const roomList = rooms.map((room) => {
    ...
    return (
      <circle
        ...
        stroke="black"
        strokeWidth="1"
        onClick={(event) => onRoomClick(event, room, imageRenderProps)}
        style={{ cursor: 'pointer' }}
      />
    )
  });
  if (markedRoom) {
    const { x: markedX, y: markedY } = markedRoom.geometry;
    index++;
    roomList.push(
      <circle
        key={index}
        cx={markedX * imageRenderProps.scale + imageRenderProps.x}
        cy={markedY * imageRenderProps.scale + imageRenderProps.y}
        r="2"
        fill="red"
        stroke="black"
        strokeWidth="1"
  ...
  console.log('Length of lineList:', lineList.length);
  return (
    <>
      {lineList}
      {roomList}
